...
import { ArrowLeft, Building2, Check, X, Edit } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-display font-bold text-white">Empresas</h1>
              <p className="text-dark-400 text-sm">{empresas.length} empresas cadastradas</p>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-dark-700 text-left">
                  ...
                  <th className="px-4 py-3 text-dark-400 font-medium">Usuário</th>
                  <th className="px-4 py-3 text-dark-400 font-medium text-center">Ativo</th>
                </tr>
              </thead>
              <tbody>
                {empresas.map((emp: Empresa) => (
                  <tr key={emp.id} className="border-b border-dark-800 hover:bg-dark-700/30">
                    <td className="px-4 py-3 text-dark-500">{emp.id}</td>
                    <td className="px-4 py-3 text-white font-medium">{emp.empresa}</td>
                    <td className="px-4 py-3 text-dark-300">{emp.user || '-'}</td>
